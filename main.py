...
numbers= [selector(data, window).shape[0]]

for i in off_zones:
    sempl = selector(data, i, en_th=1e2)
    numbers.append(sempl.shape[0])
    plt.plot(sempl['GaLon'], sempl['GaLat'], 'g+')
    plt.text(i[0,0], i[0,1], str(numbers[-1]))
    plt.plot(i[:,0], i[:, 1], 'r.', label = 'off-zones')
